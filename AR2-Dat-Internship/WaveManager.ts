import { BulletID, EnemyID, EnemyType, ENEMY_EXP_DEFAULT, EXPERIENCE_ITEM_QUANTITY_MAP, HEAL_ITEM_CHANGE_MAP, MAP_ID, GOLD_ITEM_QUANTITY_MAP,stageMultipliers } from "Enemy_Const";
...
export class WaveObserver {
  ...
  public OnCollectGold?: (expPercent: number) => void;
...
export class WaveManager extends ObserverManager<WaveObserver> implements IWaveObserver {
  ...
  public async InitialWave(wave: number) {
    ...
    this.waveConfigManager.GetWaveBulletsNeed(this.currentWave + 2).forEach((value, key) => { // acquire bullets for the next 2 waves
  ...
  public async FirstWaveHandle() {
    ...
    this.waveConfigManager.GetWaveBulletsNeed(this.currentWave).forEach((value, key) => { // acquire bullets for first wave
      this.AddProcessQueue(key, value);
    });

    this.waveConfigManager.GetWaveBulletsNeed(this.currentWave + 1).forEach((value, key) => { // acquire bullets for second wave
      this.AddProcessQueue(key, value);
    });
 ...
 private async DropLootItem(enemy: Enemy) {
  ...
  private async CollectGoldItems(): Promise<void> {
    ...
    const totalGold = ((GoldBig.length * ENEMY_GOLD_BOSS_DEFAULT) +
      (GoldMedium.length * ENEMY_GOLD_BOSS_DEFAULT) + (GoldSmall.length * ENEMY_GOLD_DEFAULT))* stageMultiplier;
    const player = this.servieLocator.Resolve<CharacterManager>(CharacterManager).Player;

    GoldManager.AddGold(totalGold,player);
    const flyPromises = lootGoldItems.map(item => item.ConnectFlyToTargetToProcessUpdate(player));
    await Promise.all(flyPromises);
    this.DispatchEvent((observer) => observer.OnCollectGold?.(GoldManager.GetGold(player)));
